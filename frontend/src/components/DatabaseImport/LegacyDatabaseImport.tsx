...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success('Database importert! Laster inn på nytt…');
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(`Import mislyktes: ${error}`);
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success('Database initialisert! Starter appen…');
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(`Initialisering mislyktes: ${error}`);
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                <>
                  ...
                  <span>{importState === 'selecting' ? 'Velger…' : 'Finner database…'}</span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>Bla etter database</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">Database funnet!</p>
