...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError('Kunne ikke laste lydenheter. Sjekk systemlydinnstillingene dine.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError('Ingen mikrofoner funnet for overvåking');
        return;
      }
      ...
      setError('Kunne ikke starte lydnivåovervåking');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">Lydenheter</h4>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder="Velg mikrofon" />
            </SelectTrigger>
            <SelectContent>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Ingen mikrofoner funnet</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">Mikrofonnivåer:</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder="Velg systemlyd" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">Standard systemlyd</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Ingen systemlydenheter funnet</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>Mikrofon:</strong> Tar opp stemmen din og omgivelseslyd</p>
        <p>• <strong>Systemlyd:</strong> Tar opp datalyd (musikk, samtaler osv.)</p>
        {isMonitoring && (
          <p>• <strong>Mikrofonnivåer:</strong> Grønn = bra, Gul = høy, Rød = for høy</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>Tips:</strong> Klikk «Test mikrofon» for å sjekke at mikrofonen virker</p>
        )}
      </div>
    </div>
  );
