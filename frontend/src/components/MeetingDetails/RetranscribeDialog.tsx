      ...
      const unlistenComplete = await listen<RetranscriptionResult>(
        ...
        async (event) => {
          if (event.payload.meeting_id === meetingId) {
            ...
            setIsProcessing(false);
            toast.success(
              `Retranskripsjon fullført! ${event.payload.segments_count} segmenter opprettet.`
            );
  ...
  const handleCancel = async () => {
    if (isProcessing) {
      try {
        ...
        setIsProcessing(false);
        setProgress(null);
        toast.info('Retranskripsjon avbrutt');
      } catch (err) {
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isProcessing
              ...
              : error
                ? 'Det oppstod en feil under retranskripsjon'
                : 'Behandle lyden på nytt med andre språkinnstillinger'}
          </DialogDescription>
        </DialogHeader>
            ...
            !isParakeetModel ? (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Velg et bestemt språk for bedre nøyaktighet, eller bruk automatisk gjenkjenning
                </p>
              </div>
            ) : (
              <div className="space-y-3">
